...
"""
    calculate_eigenvalues!(factorization::KrylovFactorization, convergenceinfo::ConvergenceInfo, valsmap!::Function, vecs::AbstractMatrix, vals::AbstractVector) -> nothing

Recompute eigenvalues from extracted eigenvectors as Rayleigh quotients under
the original operator `valsmap!`.

Mutates `vals` in-place for the converged subset.
"""
function calculate_eigenvalues!(
    ...
    vec_eltype = eltype(vecs)
    val_eltype = eltype(vals)

    Hvec = factorization.pu.Vector{vec_eltype}(undef, size(vecs,1))

    converged = convergenceinfo.converged
    vals_ = view(vals, 1:converged)
    vecs_ = view(vecs, :, 1:converged)
    vals_cpu = zeros(val_eltype, converged)

    for i in 1:converged
        vec = view(vecs_, :, i)
        valsmap!(Hvec, vec)
        vals_cpu[i] = real(dot(vec, Hvec))
    end

    # vals_ .= CuVector(vals_cpu)
    vals_[1:end] .= factorization.pu.Vector{val_eltype}(vals_cpu)
    return nothing
end
...
#     vals_host = Vector{eltype(vals)}(undef, converged)   # Vector to hold comput ed eigen values
...
#     Hvec = factorization.pu.Vector{eltype(vecs)}(undef, size(vecs,1))
