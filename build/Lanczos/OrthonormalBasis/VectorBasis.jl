"""
    VectorBasis{E}(maxdim::Int, x0::AbstractVecOrMat, pu::ProcessingUnit)

Vector-of-arrays basis storage backend for Lanczos vectors/blocks.
"""
mutable struct VectorBasis{T,E} <: OrthonormalBasis
    ...
    function VectorBasis{E}(maxdim::Int, x0::T, ::ProcessingUnit) where {T<:AbstractVecOrMat, E<:Number}
...
"""
    add!(basis::VectorBasis, v) -> nothing

Append a vector/block to basis storage, shifting out oldest entries when full.
"""
...
"""
    all(basis::VectorBasis)

Return currently stored basis entries.
"""
...
"""
    all_withoutlasttwo(basis::VectorBasis)

Return basis entries excluding the most recent two blocks.
"""
