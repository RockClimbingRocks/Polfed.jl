...
"""
    quantile_from_dos(ρ::Function, q::Real) -> Real

Return the energy corresponding to quantile `q` of the DoS distribution.

`q` is clamped to `[0, 1]` and the value is solved via bisection over the DoS
integral in `[-1, 1]`.
"""
function quantile_from_dos(ρ::Function, q::Real)
    q_clamped = clamp(q, 0.0, 1.0)
    total, _ = quadgk(ρ, -1.0, 1.0; atol=1e-12, rtol=1e-12)
    target = bisection(x -> (quadgk(ρ, -1.0, x; atol=1e-12, rtol=1e-12)[1] - q_clamped * total), -1.0, 1.0; tol=1e-8)
    return target
end
...
- `TargetMaxDoS`, `TargetMiddle`, `TargetQuantile`, `TargetOffset`, `TargetRescaled`,
  and `TargetAbsolute`.
"""
function resolve_target!(transform_plan::TransformPlan, mapping_plan::MappingPlan, ρ::Function)
    ...
    elseif spec isa TargetQuantile
        transform_plan.target = quantile_from_dos(ρ, spec.q)
    elseif spec isa TargetOffset
        peak = findmaximaldensity(ρ)
        transform_plan.target = peak + spec.frac * (1.0 - peak)
