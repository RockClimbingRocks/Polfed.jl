...
@everywhere function perform_hybrid_task(task_id)
    ...
    @threads for i in 1:2
        ...
        mat_sim = mat + mat'  # Make it symmetric
        BLAS.set_num_threads(1)  # Prevent BLAS from oversubscribing
        println("            doing work!")
        x = eigen(mat_sim)  # Some CPU-intensive work
    end
...
results = @time pmap(perform_hybrid_task, tasks_to_run)
