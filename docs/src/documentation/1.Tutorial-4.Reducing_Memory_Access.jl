...
# The question is, how to now pass in the rescaled mapping into [`polfed`](@ref) function? All of the additional things regarding the spectral transformation can be passed in via the `spectral_config` keyword argument, that accepts the [`SpectralTransformConfig`](@ref) struct. Here we need to set the `f!_rescaled`, this we can do as follows:
...
# Here unnecessary memory access can be avoided by updating directly the vector ``b_i``, doing all of the operations at once. With that in mind we can write our own clenshaw reocurence and final sumation function that avoids unnecessary memory access. Here is how such function would look like:
