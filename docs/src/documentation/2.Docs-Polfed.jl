...
#     which=:LR, # Which eigenvalues to target (default: :LR)
# )
...
# - How to change part of the spectrum to sort (e.g. `:SR`, `largest`, `:SR_by_magnitude`, `:largest_by_magnitude`), this is hendled with `which` field ([`see example`](@ref target_different_parts_of_the_spectrum)), that is later passed on to eigen sorter-er.
