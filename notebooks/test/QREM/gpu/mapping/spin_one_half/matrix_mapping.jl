...
"""
function qrem_map_mat_kernel_1d_spin_half(Y::CuDeviceMatrix{T}, X::CuDeviceMatrix{T}, Leff::Int, diag::CuDeviceVector{T}, hx::T, basis_length::Int, matrixelements::Int) where {T<:Float64}
    ...
    @inbounds @fastmath if i <= matrixelements
        ...
        Y[i] = d*x + offdiag_val*hx/2 #store the result to finial array
    end
...
function qrem_map_mat_kernel_2d_spin_half(Y::CuDeviceMatrix{T}, X::CuDeviceMatrix{T}, Leff::Int, diag::CuDeviceVector{T}, hx::T, basis_length::Int) where {T<:Real}
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = threadIdx().y
