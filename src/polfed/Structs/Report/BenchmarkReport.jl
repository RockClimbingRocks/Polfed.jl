...
mutable struct MemoryReport
    total_mem_used::Float64
    total_mem_reserved::Float64
    basis_mem_used::Float64
    basis_mem_reserved::Float64
    device_type::Symbol  # :cpu or :gpu
...
struct BenchmarkReport
    ...
    function BenchmarkReport(fact_report::FactorizationReport, total_wt::Real, total_ct::Real)
        # --- Timings ---
        ...
        function mem_report(vecdim, ncols_reserved, ncols_needed, eltyp, device::Symbol)
            ...
            return MemoryReport(
                total_mem_used / 1024^2,
                total_mem_reserved / 1024^2,
                basis_mem_used / 1024^2,
                basis_mem_reserved / 1024^2,
                device
            )
        end
        ...
        vecdim = fact_report.krylovbasisdim_history[end]  # last recorded dimension
        eltyp = ComplexF64  # TODO: auto-detect?
        ...
        elseif basistype <: MatrixBasis
            ...
            if fact_report.pu isa CPU
                memory_cpu = mem_report(vecdim, fact_report.ncols_cpu_reserved, fact_report.ncols_cpu_needed, eltyp, :cpu)
            elseif fact_report.pu isa GPU
                memory_gpu = mem_report(vecdim, fact_report.ncols_gpu_reserved, fact_report.ncols_gpu_needed, eltyp, :gpu)
            else
...
function display_report(report::BenchmarkReport)


    fact_tot_wt = sum(report.fact_wt) 
    ...
    percentages_wt = [t / fact_tot_wt * 100 for t in report.fact_wt]
    ...
    fact_tot_ct = sum(report.fact_ct) 
    ...
    percentages_ct = [t / fact_tot_ct * 100 for t in report.fact_ct]
    ...
    if show_timings
        ...
        println("- Total polfed run took: $(tot_wt_u) $units_wt (walltime), $(tot_ct_u) $units_ct (CPU time)")
    ...
    if show_memory && !isnothing(report.memory_cpu)
        println(  "\e[1;34mMemory usage (CPU):\e[0m")
    ...
    if show_memory && !isnothing(report.memory_gpu)
        println(  "\e[1;34mMemory usage (GPU):\e[0m")
        println("- Total memory used: $(report.memory_gpu.total_mem_used) GB, reserved: $(report.memory_gpu.total_mem_reserved) GB")
        println("- Basis memory used: $(report.memory_gpu.basis_mem_used) GB, reserved: $(report.memory_gpu.basis_mem_reserved) GB")
    end
