...
function definetransformation!(
    ...
    @unpack coefficients, polynomialtype, normalization, target, parallelization, order, parallelization, f!_rescaled = spectral_transform
    hilbertspacedim = size(x0,1)

    # get the normalization
    transform = Clenshaw(polynomialtype, n -> coefficients(target,n), order, f!_rescaled, hilbertspacedim, E)
    norm_ = 1/transform(target)*normalization
    coefficients_normalized(n::Int) = coefficients(target,n) * norm_


    clenshawtransform = define_clenshawtransformation(spectral_transform, coefficients_normalized, hilbertspacedim, E)
