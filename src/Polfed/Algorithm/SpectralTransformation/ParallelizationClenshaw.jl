...
- `MulColsParallel` on CPU:
  - `nt_per_col == 1`: threaded over columns,
  - `nt_per_col > 1`: direct whole-block call so optimized kernels can split
    each column into row chunks on one thread team.
...
"""CPU direct-call overload used when `MulColsParallel` keeps shared block buffers."""
function clenshaw(
    transform::ClenshawType,
...
"""
function get_b_storage(parallel::MulColsParallel, _::ProcessingUnit, x0::AbstractVector)
    return [similar(x0) for _ in 1:3]
end

function get_b_storage(parallel::MulColsParallel, pu::ProcessingUnit, x0::AbstractMatrix)
    parallel.nt_per_col > 1 && (return [similar(x0) for _ in 1:3])

    E = eltype(x0)
    hilbertspacedim = size(x0, 1)
    s = size(x0, 2)

    return [[pu.Vector{E}(undef, hilbertspacedim) for _ in 1:3] for _ in 1:s]
end
